...
USE YOMOML     , ONLY : OML_MAX_THREADS, OML_MY_THREAD, OML_IN_PARALLEL
...
IF (LLhook_ok) CALL DR_HOOK('ECSORT:INT_SORTING_METHOD',0,ZHOOK_HANDLE)
ITID = OML_MY_THREAD()
...
IF (LLhook_ok) CALL DR_HOOK('ECSORT:INT_SORTING_METHOD',1,ZHOOK_HANDLE)
...
REAL(KIND=JPRB) :: ZHOOK_HANDLE
IF (LHOOK) CALL DR_HOOK('ECSORT:STR_SORTING_METHOD',0,ZHOOK_HANDLE)
clnew = cdnew
CALL toupper(clnew)
select case (clnew)
case ('RADIX', 'RADIXSORT')
...
IF (LHOOK) CALL DR_HOOK('ECSORT:STR_SORTING_METHOD',1,ZHOOK_HANDLE)
...
INTEGER(KIND=JPIM) :: i, n
REAL(KIND=JPRB) :: ZHOOK_HANDLE
IF (LHOOK) CALL DR_HOOK('ECSORT:ADJUST_INDEX',0,ZHOOK_HANDLE)
if (index_adj /= 0) then
  n = size(index)
  do i=1,n
...
IF (LHOOK) CALL DR_HOOK('ECSORT:ADJUST_INDEX',1,ZHOOK_HANDLE)
